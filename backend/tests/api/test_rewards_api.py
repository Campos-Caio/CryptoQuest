...
class TestRewardsAPI:
 ...
 async def test_award_mission_completion(self, mock_reward_service, mock_current_user):
     ...
     async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
         response = await client.post("/rewards/award/mission?user_id=test_user&mission_id=test_mission&score=100.0&mission_type=daily")
 ...
 async def test_invalid_badge_id(self, mock_badge_repo, mock_current_user):
     ...
     async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
         ...
         assert response.status_code == 404 # Redirect (trailing slash)
