...
async def override_get_current_user():
 return FirebaseUser(uid="test_uid_", name="Test User", email="test@exemple.com")

# -- Testes --
async def test_register_user_sucess(mocker, mock_user_profile):
 """
 Testa o endpoint de registro de usuario com sucesso (0 Created)
 Passado o usuario de teste
 """
 class MockAuthService:
     async def register_user(self, user_data):
         return (
             FirebaseUser(uid='test_uid_', email='test@example.com', name='Test User'), 
             mock_user_profile
         )
